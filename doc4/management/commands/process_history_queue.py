...
from doc4.models import Repository, Snapshot, Queue, RepositoryHistory, Package

cmd_name = __name__.split('.')[-1]

queue_query = """
select doc4_snapshots.id
from  doc4_queue,  doc4_snapshots
where doc4_queue.snapshot_id=doc4_snapshots.id and action='history-store' and state='queued' and repository_id=%d
...
class Command(BaseCommand):
    ...
    args = ''
    help = """store the list of packages contained in the repository
    Examples:
    %s --id=14
